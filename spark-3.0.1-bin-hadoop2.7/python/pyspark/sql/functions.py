...
def nanvl(col1, col2):
    ...
    >>> df = spark.createDataFrame([(1.0, float('nan')), (float('nan'), 2.0)], ("a", "b"))
    >>> df.select(nanvl("a", "b").alias("r1"), nanvl(df.a, df.year).alias("r2")).collect()
...
def greatest(*cols):
    ...
    >>> df.select(greatest(df.a, df.year, df.c).alias("greatest")).collect()
    [Row(greatest=4)]
    """
    if len(cols) < 2:
...
def least(*cols):
    ...
    >>> df = spark.createDataFrame([(1, 4, 3)], ['a', 'b', 'c'])
    >>> df.select(least(df.a, df.year, df.c).alias("least")).collect()
    [Row(least=1)]
    """
    if len(cols) < 2:
...
def format_string(format, *cols):
    ...
    >>> df.select(format_string('%d %s', df.a, df.year).alias('v')).collect()
    [Row(v=u'5 hello')]
    """
    sc = SparkContext._active_spark_context
...
def concat(*cols):
    ...
    >>> df.select(concat(df.a, df.year, df.c).alias("arr")).collect()
